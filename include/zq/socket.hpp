...
// but to be technically correct, this deleter is needed for the unique_ptr
  ...
  struct Socket {
    ...
    [[nodiscard]]
    tl::expected<size_t, ErrMsg> send(const TypedMessage& msg) {
      size_t bytes_sent = 0;
      auto rc = zmq_send(socket_ptr.get(), msg.type.data(), msg.type.size(), ZMQ_SNDMORE);
      if (rc < 0) {
        return tl::make_unexpected(currentErrMsg());
      }
      bytes_sent += static_cast<size_t>(rc);
      rc = zmq_send(socket_ptr.get(), msg.payload.data(), msg.payload.size(), ZMQ_DONTWAIT);
      if (rc < 0) {
        return tl::make_unexpected(currentErrMsg());
      }
    ...
    std::optional<tl::expected<TypedMessage, std::runtime_error>>
      ...
      auto rc = zmq_msg_recv(std::addressof(typed_message.type.msg), socket_ptr.get(), ZMQ_DONTWAIT);
      ...
      if (more) {
        rc = zmq_msg_recv(std::addressof(typed_message.payload.msg), socket_ptr.get(), 0);
        if (rc == -1) {
          return tl::make_unexpected(currentZmqRuntimeError());
        }
      }
  ...
  [[nodiscard]]
  inline
    tl::expected<size_t, std::runtime_error>
    subscribe(Socket& subscriber, std::initializer_list<std::string_view> topic_filters) {
    // subscribe to all topics, todo, maybe overload this function with 1 or 2 arguments
      {
        int val = 0;
        size_t size = sizeof(val);
        int rc = zmq_getsockopt(subscriber.socket_ptr.get(), ZMQ_TYPE, std::addressof(val), std::addressof(size));
        if (rc != 0) {
          return tl::make_unexpected(currentZmqRuntimeError());
        }
        if (val != ZMQ_SUB) {
          return tl::make_unexpected(std::runtime_error("socket is not a subscriber"));
        }
      }

      if (topic_filters.size() == 0) {
        auto rc = zmq_setsockopt(subscriber.socket_ptr.get(), ZMQ_SUBSCRIBE, "", 0);
        if (rc != 0) {
          return tl::make_unexpected(currentZmqRuntimeError());
        }
      ...
      for (auto&& t : topic_filters) {
        auto rc = zmq_setsockopt(subscriber.socket_ptr.get(), ZMQ_SUBSCRIBE, t.data(), t.size());
        if (rc != 0) {
          return tl::make_unexpected(currentZmqRuntimeError());
        }
        topicCount++;
